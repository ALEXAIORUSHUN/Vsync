...
admins = {
    --'steam:76561199142094256',
    'discord:1187013223761858602', -- Discord ID Szükséges ide, ha hozzáakarod adni a következő embert, akkor '', után folytatod tovább
    'discord:1187013223761858602'
...
RegisterCommand('freezetime', function(source, args) --Megtudod állítani az időt
    if source ~= 0 then
        if isAllowedToChange(source) then
            freezeTime = not freezeTime
            if freezeTime then
                TriggerClientEvent('dopeNotify2:Alert', source, "Idő",  "megállítva", 5000, 'info')
            else
                TriggerClientEvent('dopeNotify2:Alert', source, "Idő", "elindítva", 5000, 'info')
            end
        else
...
RegisterCommand('freezeweather', function(source, args) -- Időjárást tudod megállítani
    if source ~= 0 then
        if isAllowedToChange(source) then
            DynamicWeather = not DynamicWeather
            if not DynamicWeather then
                TriggerClientEvent('dopeNotify2:Alert', source, "Idő Járás", "megállítva", 5000, 'info')
            else
                TriggerClientEvent('dopeNotify2:Alert', source, "Idő Járás", "elindítva", 5000, 'info')
            end
        else
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Error: ^1You are not allowed to use this command.')
        end
...
RegisterCommand('weather', function(source, args) -- /weather extrasunny pl.
    if source == 0 then
...
RegisterCommand('time', function(source, args, rawCommand) -- /time 12 00 pl. 
    if source == 0 then
